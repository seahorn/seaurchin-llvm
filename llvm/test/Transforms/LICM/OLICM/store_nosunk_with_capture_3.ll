; NOTE: This tests that if any pointer of the alias set is captured in loop (inside cond_inc) and 
; NOTE: func call to cond, then the safe ptr is assumed to escape through the call if the call has args
...
; CHECK-NEXT:   [[ESC_PTR:%.*]] = alloca %indirect_strct, align 8
; CHECK-NEXT:   [[FIELD_PTR:%.*]] = getelementptr inbounds %indirect_strct, ptr [[ESC_PTR]], i32 0, i32 0
; CHECK-NEXT:   br label %[[LOOP_BLOCK:.*]]
; CHECK:      [[EXIT_BLOCK:.*]]:{{.*}}
; CHECK-NEXT:   store ptr [[BORROW_GEP]], ptr [[FIELD_PTR]], align 8
; CHECK-NEXT:   ret void

start:
  %0 = getelementptr inbounds i8, ptr %b, i64 4, !ownsem !13
  %esc_ptr = alloca %indirect_strct, align 8
  %field_ptr = getelementptr inbounds %indirect_strct, ptr %esc_ptr, i32 0, i32 0
  br label %loop 
...
loop:                                              ; preds = %bb3, %start
  ...
  %_5 = load i32, ptr %0, align 4, !dbg !49
  store ptr %0, ptr %field_ptr, align 8 ; capture inside loop
  %_6 = tail call noundef zeroext i1 @cond(ptr %esc_ptr) #2, !dbg !47
  %2 = add i32 %_5, 1, !dbg !49
...
;declare noundef zeroext i1 @cond(ptr) unnamed_addr #1 memory(argmem: readwrite)

declare noalias ptr @malloc(i64)

define noundef zeroext i1 @cond(ptr %esc_ptr) {
entry:
  ; Get pointer to the first field of %esc_ptr (which is a pointer itself)
  %field_ptr = getelementptr inbounds %indirect_strct, ptr %esc_ptr, i32 0, i32 0

  ; Allocate memory (say 8 bytes)
  %malloc_ptr = call noalias ptr @malloc(i64 8)

  ; Store the allocated pointer into the field
  store ptr %malloc_ptr, ptr %field_ptr, align 8
