; NOTE: This tests that if any pointer of the alias set is captured between func entry (cond_inc) and 
; NOTE: func call to cond, then the safe ptr is assumed to not escape through the call if the call has *NO* args
; NOTE: We want promotion but status quo does not promote - possibly because cond is supposed to have access to alias under C model 
; NOTE: but should not if captured aliases are "safe" and there is no explicit transfer through args
; NOTE: This test should be enabled once alias analysis is improved.
...
; CHECK-NEXT:   [[ESC_PTR_FIELD:%.*]] = getelementptr inbounds %indirect_strct, ptr [[ESC_PTR]], i32 0, i32 0
; CHECK-NEXT:   store ptr [[BORROW_GEP]], ptr [[ESC_PTR_FIELD]], align 8
; CHECK-NEXT:   [[LOAD_VAL:.*]] = load i32, ptr [[BORROW_GEP]], align 4
; CHECK-NEXT:   br label %[[LOOP_BLOCK:.*]]
; CHECK:      [[EXIT_BLOCK:.*]]:{{.*}}
; CHECK-NEXT:   [[STORE_VAL:%.*]] = phi i32 [ [[PHI_VAL_FROM_LOOP:%.*]], [[LOOP_CONT:%.*]] ]
...
start:
  ...
  store ptr %0, ptr %field_ptr, align 8
  br label %loop 
